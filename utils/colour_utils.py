...
def generate_color_map(terrain_data, blend_toggle=False, variation_toggle=True):
    """
    Generates a color-coded terrain map based on elevation, biomes, and regions.
    """
    rows, cols = terrain_data["elevation_map"].shape
    colour_map = np.zeros((rows, cols, 3), dtype=np.uint8)

    for r in range(rows):
        for c in range(cols):
            region = terrain_data["region_map"][r][c]
            color = REGION_COLORS.get(region, (0, 255, 0))  # Default to green
            colour_map[r, c] = color

            if region == "water":

                blend_factor = (terrain_data["elevation_map"][r, c] + 1) / 2  # Normalize to 0-1 range for water
                colour_map[r, c] = blend_colors(colour_map[r, c], (190, 190, 255), blend_factor)


            elif variation_toggle:

                colour_map[r, c] = blend_colors(colour_map[r, c], (0,0,0), terrain_data["steepness_map"][r, c] * 1.4)
                colour_map[r, c] = blend_colors(colour_map[r, c], (0,0,0), terrain_data["elevation_map"][r, c] / 2)
