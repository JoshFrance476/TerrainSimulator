...
class Slider:
    def __init__(self, font, min_val, max_val, bar_width, default_value = -1):
        self.min = min_val
        self.max = max_val
    ...
    def draw(self, screen, x, y):
        self.bar_rect = pygame.Rect(x+self.bar_x_offset, y, self.bar_width, self.height)

        # value → pixel
        t = (self.value - self.min) / (self.max - self.min)
        pin_x = x + t * self.bar_width

        self.pin_rect = pygame.Rect(pin_x + self.bar_x_offset-10, y - 5, 20, 20)
        ...
        screen.blit(value_text, (x, y-3))

        min_text = self.font.render(str(self.min), True, (30,30,30))
        screen.blit(min_text, (x+self.bar_x_offset-20, y-3))

        max_text = self.font.render(str(self.max), True, (30,30,30))
        screen.blit(max_text, (x+self.bar_width+self.bar_x_offset+10, y-3))
