...
def simulate_init():
    global desiribility_map, desiribility_display, colour_display, population_display, outline_display, elevation_display, traversal_cost_map, sea_map, river_map, territory_display, cities_list, population_map
    colour_display, desiribility_map, elevation_map, traversal_cost_map, sea_map, river_map = generate_initial_maps()
...
def initialise_cities(population_map):
    ...
    for uid, (_, (r, c)) in enumerate(anchor_location_list):
        AnchorPoint((r, c),random.randint(*config.CITY_PROSPERITY_RANGE), uid+1)
